...
## UE-to-use-case mapping (assigned rounded 16-UE realization)
...
See `traffic_volume_summary.csv` for full numbers. **The measured traffic-volume-by-bytes percentages do not match the assigned target percentages, and this is expected, not a fabrication:**
...
The UE-share (count) percentages track the assigned target reasonably closely (they are the rounded 16-UE realization, as instructed). **The traffic-volume-by-bytes percentages do not**, because the actual byte volume generated by each profile in this run was driven entirely by the traffic-generation parameters chosen (ping packet count/size vs iperf3 duration/bitrate), not by any attempt to hit the target volume shares:
...
9. Traffic-volume-by-bytes percentages do not match the assigned target distribution (see Result 4) -- this is a direct consequence of the chosen per-profile traffic parameters, not an error to be corrected by adjusting reported figures.
